...
export default function PatientDashboard() {
  ...
  const fetchPatientData = useCallback(async () => {
    try {
      ...
      // Fetch profile data
      const profileResponse = await fetch(`https://${process.env.REACT_APP_SERVER_IP}/api/patient/profile`, {
      ...
      const prescriptionsResponse = await fetch(`https://${process.env.REACT_APP_SERVER_IP}/api/patient/prescriptions`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const visitsResponse = await fetch(`https://${process.env.REACT_APP_SERVER_IP}/api/patient/visits`, {
        headers: { Authorization: `Bearer ${token}` },
      });
